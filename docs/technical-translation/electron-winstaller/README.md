...
用于构建Windows安装程序的NPM模块
[Electron](https://github.com/electron/electron) 应用程序用
...
运行后你就会有一个 `.nupkg`, 一个 `RELEASES` 文件, 并且有一个 `.exe` 安装文件 `outputDirectory` 文件夹中。
对于配置项下给定的每个多任务目标。
...
| --------------------- | -------- | ----------- |
| `appDirectory`        | Yes      | 你的 Electron 应用程序文件路径 |
